...
def run_sequential(args, logger):
    ...
    args.unit_dim = env_info.get("unit_dim", env_info["obs_shape"])
    args.obs_shape = env_info["obs_shape"]

    if args.mac == "abim_mac":
        # 你的算法是序列混合动作，vshape 取决于 action_dims_list 的长度
        num_action_components = len(args.action_dims_list)
        action_vshape = (num_action_components,)
    else:
        action_vshape = (1,)
